...
import { GitHubRelease } from '../../downloads/downloads.model';
import { isHeadlessRelease } from '../../downloads/release.utils';
  ...
  startIndexing(sections: DocSection[]): void {
    ...
    this.loadSearchIndex().subscribe({
      next: (index) => {
        ...
        console.debug(`[DocService] Loaded search index: ${Object.keys(index).length} items.`);
      },
      error: (err: unknown) => {
